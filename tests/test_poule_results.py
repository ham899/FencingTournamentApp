from poule_results import EntryPouleResult, PouleResult, PouleResults
...
def test_entry_poule_result_creation_no_defaults(entry1):
    epr = EntryPouleResult(entry=entry1, poule_id=1, matches_fenced=6, victories=4, touches_scored=21, touches_received=18)
    assert epr.entry == entry1
    assert epr.poule_id == 1
    assert epr.matches_fenced == 6
...
def test_entry_poule_result_invalid_types(entry1):
    ...
    with pytest.raises(TypeError):
        EntryPouleResult(entry=entry1, poule_id=1, matches_fenced='6')
    with pytest.raises(TypeError):
...
def test_entry_poule_result_invalid_values(entry1):
    ...
    with pytest.raises(ValueError):
        EntryPouleResult(entry=entry1, poule_id=-1)
    with pytest.raises(ValueError):
        EntryPouleResult(entry=entry1, poule_id=1, matches_fenced=-5)
    with pytest.raises(ValueError):
...
def test_entry_poule_result_equality():
    epr1 = EntryPouleResult(entry=TournamentEntry(id=12, tournament_id=1, fencer=Fencer(id=12, display_name='Yvonne')), poule_id=1, matches_fenced=5, victories=3, touches_scored=15, touches_received=10)
    epr2 = EntryPouleResult(entry=TournamentEntry(id=12, tournament_id=1, fencer=Fencer(id=12, display_name='Yvonne')), poule_id=1, matches_fenced=5, victories=3, touches_scored=15, touches_received=10)
    assert epr1 == epr2

def test_entry_poule_result_inequality(entry1):
    epr1 = EntryPouleResult(entry=entry1, poule_id=1, matches_fenced=5, victories=4, touches_scored=18, touches_received=9)
    epr2 = EntryPouleResult(entry=entry1, poule_id=1, matches_fenced=5, victories=3, touches_scored=15, touches_received=10)
...
def test_poule_result_creation_no_defaults(entries):
    ...
    for i, entry in enumerate(entries):
        eprs.append(EntryPouleResult(entry=entry, poule_id=1, matches_fenced=1, victories=1, touches_scored=1, touches_received=1))

    pr = PouleResult(poule_id=1, results=eprs)
    assert pr.poule_id == 1
    assert len(pr.results) == 7
    for i in range(7):
        assert pr.results[i].entry == entries[i]
        assert pr.results[i].poule_id == 1
        assert pr.results[i].matches_fenced == 1
...
def test_poule_result_creation_invalid_types(entry1):
    with pytest.raises(TypeError):
        PouleResult(poule_id='1')
    with pytest.raises(TypeError):
        PouleResult(poule_id=1, results=True)
    with pytest.raises(TypeError):
        PouleResult(poule_id=1, results=[EntryPouleResult(entry=entry1, poule_id=1, matches_fenced=5, victories=3, touches_scored=15, touches_received=10), 'random string'])

def test_poule_result_creation_invalid_values():
    with pytest.raises(ValueError):
        PouleResult(poule_id=0)
    with pytest.raises(ValueError):
        PouleResult(poule_id=-1)

def test_poule_result_add_entry_poule_result(entry1):
    pr = PouleResult(poule_id=1)
    epr = EntryPouleResult(entry=entry1, poule_id=1, matches_fenced=4, victories=2, touches_scored=11, touches_received=9)
    ...
    assert pr.results[0].matches_fenced == 4
...
def test_poule_result_init_entry_poule_results(entries):
    ...
    for i in range(n):
        assert pr.results[i].entry == entries[i]
        assert pr.results[i].poule_id == 1
        assert pr.results[i].matches_fenced == 0
...
def test_poule_result_calculate_standings(entries):
    ...
    eprs = [None] * 7
    eprs[0] = EntryPouleResult(entry=entries[0], poule_id=1, matches_fenced=6, victories=3, touches_scored=22, touches_received=21)
    eprs[1] = EntryPouleResult(entry=entries[1], poule_id=1, matches_fenced=6, victories=1, touches_scored=12, touches_received=28)
    eprs[2] = EntryPouleResult(entry=entries[2], poule_id=1, matches_fenced=6, victories=6, touches_scored=30, touches_received=13)
    eprs[3] = EntryPouleResult(entry=entries[3], poule_id=1, matches_fenced=6, victories=4, touches_scored=24, touches_received=20)
    eprs[4] = EntryPouleResult(entry=entries[4], poule_id=1, matches_fenced=6, victories=5, touches_scored=28, touches_received=18)
    eprs[5] = EntryPouleResult(entry=entries[5], poule_id=1, matches_fenced=6, victories=1, touches_scored=18, touches_received=27)
    eprs[6] = EntryPouleResult(entry=entries[6], poule_id=1, matches_fenced=6, victories=1, touches_scored=19, touches_received=26)
...
def test_poule_results_creation_invalid_types():
    with pytest.raises(TypeError):
        PouleResults(results=True)
    with pytest.raises(TypeError):
        PouleResults(results=[EntryPouleResult(entry=TournamentEntry(id=12, tournament_id=1, fencer=Fencer(id=12, display_name='Yvonne')), poule_id=1, matches_fenced=5, victories=3, touches_scored=15, touches_received=10), 'bad value'])

def test_is_sorted_on_creation():
    epr1 = EntryPouleResult(entry=TournamentEntry(id=12, tournament_id=1, fencer=Fencer(id=12, display_name='Yvonne')), poule_id=1, matches_fenced=5, victories=3, touches_scored=15, touches_received=10)
    epr2 = EntryPouleResult(entry=TournamentEntry(id=13, tournament_id=1, fencer=Fencer(id=13, display_name='Zach')), poule_id=1, matches_fenced=5, victories=4, touches_scored=18, touches_received=9)
    epr3 = EntryPouleResult(entry=TournamentEntry(id=14, tournament_id=1, fencer=Fencer(id=14, display_name='Xavier')), poule_id=1, matches_fenced=5, victories=2, touches_scored=12, touches_received=15)
    epr4 = EntryPouleResult(entry=TournamentEntry(id=15, tournament_id=1, fencer=Fencer(id=15, display_name='Wendy')), poule_id=1, matches_fenced=5, victories=4, touches_scored=18, touches_received=9)
    prs = PouleResults(results=[epr1, epr2, epr3, epr4])
