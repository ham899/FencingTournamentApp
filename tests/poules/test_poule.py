...
def test_poule_number_matches(entries):
...
def test_poule__create_match(entries):
    ...
    m = poule._create_match(id=1, index=0, match_pair=(1, 2))
    assert m.id == 1
...
def test_poule__create_match_invalid_types(entries):
    poule = Poule(id=1, tournament_id=1, poule_number=1, entries=entries[:3])
    with pytest.raises(TypeError):
        poule._create_match(id=1, index='1', match_pair=(1, 2))
    with pytest.raises(TypeError):
        poule._create_match(id=1, index=1, match_pair=True)
...
def test_poule__create_match_invalid_values(entries):
    ...
    with pytest.raises(ValueError):
        poule._create_match(id=1, index=-1, match_pair=(1, 2))
    with pytest.raises(ValueError):
        poule._create_match(id=1, index=100, match_pair=(1, 2))
    with pytest.raises(ValueError):
        poule._create_match(id=1, index=1, match_pair=(1,))
    with pytest.raises(ValueError):
        poule._create_match(id=1, index=1, match_pair=(0, 2))
    with pytest.raises(ValueError):
        poule._create_match(id=1, index=1, match_pair=(1, 4))
...
def test_poule_get_next_match_when_no_matches_present(entries):
    poule = Poule(id=1, tournament_id=1, poule_number=1, entries=entries)
    poule.matches = None
    with pytest.raises(ValueError):
        poule.get_next_match()
...
def test_poule_record_match_result_invalid(entries):
    ...
    with pytest.raises(ValueError):
        poule.matches=None
        poule.record_match_result(index=1, score1=5, score2=1)
    
    # Regenerate matches
    poule._generate_own_matches()
...
def test_poule_calculate_results(entries):
    ...
    assert poule_results.results[0].matches == 1
    ...
    assert poule_results.results[1].matches == 1
    ...
    assert poule_results.results[2].matches == 0
    ...
    # Validate John's current results
    assert poule_results.results[0].matches == 2
    assert poule_results.results[0].victories == 1
    ...
    assert poule_results.results[1].touches_received == 5
    # Validate Hannah's current results
    assert poule_results.results[2].matches == 1
    ...
    assert poule_results.results[1].matches == 2
    assert poule_results.results[1].victories == 0
    ...
    assert poule_results.results[2].matches == 2
