...
def test__validation_location_function_name_and_method_name():
    with pytest.raises(ValueError):
        validation._validation_location(method_name='my_method', function_name='my_function')
...
@pytest.mark.parametrize('value', INVALID_INT_TYPES[1:])
...
def test_validate_int_less_than_or_equal_to_accepts_valid_values(value, max_value):
    validation.validate_int_less_than_or_equal_to(value, max_value, 'value')

@pytest.mark.parametrize(('value', 'max_value'), [(-10, -15), (0, -5), (5, 0), (10, 5), (15, 10)])
def test_validate_int_less_than_or_equal_to_rejects_invalid_values(value, max_value):
    with pytest.raises(ValueError):
        validation.validate_int_less_than_or_equal_to(value, max_value, 'value')

@pytest.mark.parametrize('value', INVALID_INT_TYPES)
def test_validate_int_less_than_or_equal_to_rejects_non_int_value(value):
    with pytest.raises(TypeError):
        validation.validate_int_less_than_or_equal_to(value, 15, 'value')

@pytest.mark.parametrize('max_value', INVALID_INT_TYPES)
def test_validate_int_less_than_or_equal_to_rejects_non_int_max_value(max_value):
    with pytest.raises(TypeError):
        validation.validate_int_less_than_or_equal_to(10, max_value, 'value')

@pytest.mark.parametrize(('value', 'min_value'), [(0, 0), (1, 0), (5, 0), (10, 5), (15, 10)])
def test_validate_int_greater_than_or_equal_to_accepts_valid_values(value, min_value):
    validation.validate_int_greater_than_or_equal_to(value, min_value, 'value')

@pytest.mark.parametrize(('value', 'min_value'), [(-5, 0), (-1, 0), (0, 1), (5, 10), (10, 15)])
def test_validate_int_greater_than_or_equal_to_rejects_invalid_values(value, min_value):
    with pytest.raises(ValueError):
        validation.validate_int_greater_than_or_equal_to(value, min_value, 'value')

@pytest.mark.parametrize('value', INVALID_INT_TYPES)
def test_validate_int_greater_than_or_equal_to_rejects_non_int_value(value):
    with pytest.raises(TypeError):
        validation.validate_int_greater_than_or_equal_to(value, 0, 'value')

@pytest.mark.parametrize('min_value', INVALID_INT_TYPES)
def test_validate_int_greater_than_or_equal_to_rejects_non_int_min_value(min_value):
    with pytest.raises(TypeError):
        validation.validate_int_greater_than_or_equal_to(10, min_value, 'value')
