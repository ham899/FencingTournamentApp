...
def test_fencer_creation_valid_max_length_name():
    name = "a" * Fencer.max_name_length
    fencer = Fencer(id=VALID_ID1, display_name=name)
    assert fencer.display_name == name
...
def test_fencer_creation_invalid_id_negative(negative_id):
    with pytest.raises(ValueError):
        Fencer(id=negative_id, display_name=VALID_NAME)

@pytest.mark.parametrize('invalid_display_name_types', [None, 1, 1.0, False, [], {}])
...
# --- Dunder Method Tests ---
def test_fencer_equality(fencer):
    fencer2 = Fencer(id=VALID_ID1, display_name='Katherine')
    assert fencer == fencer2 # Exact same fencer
    fencer3 = Fencer(id=VALID_ID1, display_name='Kate')
    assert fencer == fencer3 # Same ID, but name has changed - should still be equal

def test_fencer_inequality(fencer):
    fencer2 = Fencer(id=VALID_ID2, display_name='Daniel')
    assert fencer != fencer2 # Two completely different fencers
...
def test_fencer_update_display_name_valid_name_strip(fencer):
    fencer.update_display_name('    Catherine     ')
    assert fencer.id == VALID_ID1
    assert fencer.display_name == 'Catherine'

@pytest.mark.parametrize('invalid_display_name_type', [None, 1, 1.0, True, [], {}])
def test_fencer_update_display_name_invalid_type(fencer, invalid_display_name_type):
    with pytest.raises(TypeError):
        fencer.update_display_name(invalid_display_name_type)

def test_fencer_update_display_name_invalid_empty_input_string(fencer):
    with pytest.raises(ValueError):
        fencer.update_display_name('')

def test_fencer_update_display_name_invalid_whitespace_only_input(fencer):
    with pytest.raises(ValueError):
        fencer.update_display_name('     ')

def test_fencer_update_display_name_invalid_name_too_long(fencer):
    with pytest.raises(ValueError):
        fencer.update_display_name('abcdefghijklmnopqrstuvwxyz0123456789')
